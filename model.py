...
def build_params(seed: int, layer_sizes: list[int] = [784, 512, 512, 10]):
    mx.random.seed(seed)
    def random_layer_params(in_dim: int, out_dim: int, initializer) -> tuple[mx.array, mx.array]:
        return initializer(mx.zeros((out_dim, in_dim))), mx.zeros((out_dim,))
...
def predict_single_harmonic(params: list[mx.array], flattened_image: mx.array):
    # per-example predictions,
    # flattened_image is of dimension (d,)
    ...
    # forward
    for w, b in params[:-1]:
        outs = mx.matmul(w, x) + b
        x = relu(outs)

    final_w, final_b = params[-1]

    # currently, final_b is not used (no bias in final layer)

    deg = mx.sqrt(final_w.shape[0])
    # here, we will implement the harmonic loss as described in https://arxiv.org/abs/2502.01628:
    # d_i = ||w_i - x||_2
    # p_i = (1/d_i^n) / (sum_j (1/d_j^n))
    # loss = -log p_i

    logits = -deg * mx.log(mx.linalg.norm(final_w - x, axis=1))
    return logits - mx.logsumexp(logits)  # log(probs)

def predict_single_xent(params: list[mx.array], flattened_image: mx.array):
    # per-example predictions
    ...
    final_w, final_b = params[-1]
    logits = mx.matmul(final_w, x) + final_b
